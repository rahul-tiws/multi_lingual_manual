...
from utils.textwrap_japanese import fw_fill_ja
from utils.textwrap_vietnamese import fw_fill_vi
...
class TranslationLayoutRecovery:
    ...
    def translate_pdf(self, input_path: Union[Path, bytes], language: str, output_path: Path, merge: bool) -> None:
        ...
        """
        pdf_images = convert_from_path(input_path, dpi=self.DPI ,poppler_path=r'D:\multilingual-user-manual-main\Model\poppler-24.08.0\Library\bin')
    ...
    def _load_init(self):
        ...
        self.font_ar = ImageFont.truetype(
            os.path.join(os.getcwd(), "Amiri-Regular.otf"),
            size=self.FONT_SIZE_ARABIC,
        )
        ...
        self.translate_model_ar = AutoModelForSeq2SeqLM.from_pretrained("Helsinki-NLP/opus-mt-en-ar")
        self.translate_tokenizer_ar = AutoTokenizer.from_pretrained("Helsinki-NLP/opus-mt-en-ar")
            ...
            for ocr_results, box in zip(list_ocr_results, list_new_boxes):
                if ocr_results is not None:
                    ...
                    if len(ocr_text) > 1:
                        ...
                        # Arabic
                        if self.language == "ar":
                            translated_text = translated_text.replace("ar: ", "")
                            translated_text = translated_text.replace("ar ", "")
                            translated_text = translated_text.strip()
                        ...
                        if self.language == "ar":
                            if self._repeated_substring(translated_text): # Check repeated substring
                                processed_text = fw_fill_ja(
                                    text,
                                    width=int(
                                        (box[2] - box[0]) / (self.FONT_SIZE_ARABIC / 2)
                                    )
                                    + 1,
                                )
                            else:
        ...
        for i, t in enumerate(texts):
            ...
            if not http_res:
                if self.language == "ja":
                    ...
                    inputs = self.translate_tokenizer_ar(t, return_tensors="pt").input_ids
                    outputs = self.translate_model_ar.generate(inputs, max_length=512)
                    res = self.translate_tokenizer_ar.decode(outputs[0], skip_special_tokens=True)
            else:
                res = t
...
if __name__ == "__main__":
    ...
    obj.translate_pdf(
        language="ar",
        input_path="manual_EF494T_en-pages.pdf",
        output_path="outputs/",
        merge=False,
    )
